    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        if (num == 1) {
            ...
            if (start.sequenceNum() != 0) {
                for (int m = 0; m < model.width(); m++) {
                    for (int n = 0; n < model.height(); n++) {
                        s1 = model.get(m, n);
                        boolean t = start.sequenceNum() + 1 == s1.sequenceNum();
                        if (start.connectable(model.get(m, n)) && t) {
                            return s1;
                        }
                    }
                }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        if (num == 1) {
            ...
            if (end.predecessors() != null) {
                for (Place each: end.predecessors()) {
                    Sq newone = model.get(each.x, each.y);
                    if (newone.sequenceNum() + 1 == end.sequenceNum()) {
                        return model.get(each.x, each.y);
                    }
